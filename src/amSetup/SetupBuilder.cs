...
function previewInstaller(){const m=manifest();const pt=previewTheme(m);const pc=cssColor(m.theme?.progressColor,pt.accent);const dirs=(m.installDirectories||[]).map(d=>`<div>[folder] ${escapeHtml(d.path)}</div>`).join('')||'<div>No extra folders</div>';const comps=(m.components||[]).map(c=>`<label><input type="checkbox" checked disabled> ${escapeHtml(c.name||c.id)} ${c.required?'(required)':''}</label>`).join('')||'<label><input type="checkbox" checked disabled> Main Files</label>';const prereqs=(m.prerequisites||[]).map(p=>`<div>${escapeHtml(p.name||p.id)}</div>`).join('')||'<div>No prerequisites</div>';const title=expandTemplate(m.window?.title||'{ProductName} Setup',m),sub=expandTemplate(m.window?.subtitle||'',m),intro=expandTemplate(m.window?.introText||'',m),foot=expandTemplate(m.window?.footerText||'',m);$('previewContent').innerHTML=`<div class="setupPreview" style="max-width:${m.window?.width||720}px;min-height:${m.window?.height||460}px;background:${pt.body};color:${pt.text};display:grid;grid-template-columns:${m.window?.showSidebar!==false?'150px 1fr':'0 1fr'}"><div style="background:${pt.side};color:${pt.sideText};padding:18px 14px;border-right:6px solid ${pt.accent}"><b>${escapeHtml(m.productName||'Setup')}</b><br><small>Setup Wizard</small><div style="margin-top:28px;color:${pt.muted}">Welcome<br>License<br>Options<br>Install<br>Finish</div></div><div><div class="setupHead" style="color:${pt.title};background:${pt.head}">${escapeHtml(title)}</div><div class="setupBody"><h3>${escapeHtml(sub)}</h3><p>${escapeHtml(intro)}</p><div class="setupSplash" style="border-color:${pt.line};color:${pt.muted}">${m.branding?.showSplash?'Splash: '+escapeHtml(m.branding.splashPath||'embedded image'):'No splash'}</div><div><b>Install folder</b><br>${escapeHtml(m.defaultInstallDirectory||'{InstallDir}')}</div><div class="componentPreview">${comps}</div><div><b>Prerequisites</b>${prereqs}</div><div><b>Created folders</b><div class="folderList">${dirs}</div></div><p style="color:${pt.accent}">Shortcut: ${escapeHtml((m.shortcuts||[])[0]?.target||'none')}</p><div class="progressPreview"><div style="background:${pc}"></div></div><small style="color:${pt.muted}">${escapeHtml(foot)}</small></div></div></div>`;$('previewModal').classList.add('open');}
function previewTheme(m){const s=(m.window?.style||'').toLowerCase();if(s=='dark')return{body:'#191d22',head:'#12161b',side:'#0c1116',title:'#eef2f5',text:'#dde2e6',muted:'#9eaab4',line:'#525e6a',accent:cssColor(m.theme?.accentColor,'#41c7a5'),sideText:'#fff'};if(s=='amber')return{body:'#14110a',head:'#1f190c',side:'#30240b',title:'#ffd67a',text:'#f8e0ac',muted:'#c3a05c',line:'#77561f',accent:'#de9d2a',sideText:'#ffe8a6'};if(s=='oldschool')return{body:'#e0e0e0',head:'#c6c6c6',side:'#000080',title:'#000',text:'#000',muted:'#404040',line:'#606060',accent:'#008080',sideText:'#fff'};return{body:'#fff',head:'#f5f5f5',side:'#1c3658',title:'#000',text:'#202020',muted:'#464646',line:'#aaa',accent:cssColor(m.theme?.accentColor,'#0078d7'),sideText:'#fff'};}
function expandTemplate(s,m){return String(s||'').replaceAll('{ProductName}',m.productName||'').replaceAll('{Version}',m.version||'').replaceAll('{Identifier}',m.identifier||'').replaceAll('{Publisher}',m.publisher||'');}
