...
.pathrow{display:grid;grid-template-columns:1fr auto;gap:8px}.picker,.previewModal{position:fixed;inset:5vh 5vw;background:#151a1d;border:1px solid var(--line);border-radius:8px;box-shadow:0 18px 80px #000b;z-index:10;padding:14px;display:none;grid-template-rows:auto auto minmax(0,1fr) auto;gap:10px}.picker.open,.previewModal.open{display:grid}.previewTop{display:flex;align-items:center;gap:10px}.previewTop h3{margin-right:auto}.previewTop select{width:auto;min-width:180px}.previewStage{overflow:auto;border:1px solid var(--line);border-radius:8px;background:#0f1315;padding:18px}.picklist{overflow:auto;border:1px solid var(--line);border-radius:6px;background:#0f1315}.pickitem{display:grid;grid-template-columns:28px 1fr;gap:8px;padding:8px 10px;border-bottom:1px solid #20282d;cursor:pointer}.pickitem:hover{background:#20272b}.pickbar{display:grid;grid-template-columns:1fr auto auto;gap:8px}.selectrow{display:grid;grid-template-columns:1fr auto;gap:8px}.registry,.dirrow{display:grid;grid-template-columns:90px 1fr 120px;gap:8px;align-items:end}.setupPreview{background:var(--content);border:1px solid var(--border);box-shadow:0 16px 42px #0009;color:var(--text);font-family:Segoe UI,Arial,sans-serif;font-size:12px;overflow:hidden;display:grid;grid-template-rows:30px 1fr}.setupChrome{background:linear-gradient(#ffffff18,#00000010),var(--caption);color:var(--captionText);display:flex;align-items:center;padding:0 10px;border-bottom:1px solid var(--border);font-size:12px}.setupChromeBtns{margin-left:auto;display:flex;gap:5px}.setupChromeBtns span{width:10px;height:10px;border:1px solid #ffffff55;background:#ffffff1f}.setupClient{min-height:0;display:grid;grid-template-columns:var(--sidebarWidth) 1fr;grid-template-rows:1fr 54px}.setupSidebar{position:relative;background:linear-gradient(var(--sideGloss),var(--side));color:var(--sideText);padding:28px 12px 12px 18px;overflow:hidden}.setupSidebar:before{content:"";position:absolute;left:0;top:0;bottom:0;width:6px;background:linear-gradient(var(--accentHover),var(--accentDown))}.setupSidebar:after{content:"";position:absolute;left:6px;right:0;top:0;height:64px;background:#ffffff26}.setupSidebar b,.setupSidebar small,.setupSteps{position:relative}.setupSidebar b{display:block;font-size:17px;line-height:21px;margin-bottom:12px}.setupSidebar small{color:var(--sideMuted)}.setupSteps{display:grid;gap:16px;margin-top:48px;color:var(--sideMuted)}.setupStep{display:grid;grid-template-columns:22px 1fr;align-items:center}.setupStepDot{width:20px;height:20px;border-radius:50%;display:grid;place-items:center;color:var(--sideText)}.setupStep.active{color:var(--sideText)}.setupStep.active .setupStepDot{background:var(--accent)}.setupContent{background:var(--content);padding:20px 24px 12px;overflow:hidden}.setupTitle{height:34px;color:var(--title);font-size:20px;font-weight:700;white-space:nowrap;overflow:hidden;text-overflow:ellipsis}.setupSub{height:38px;color:var(--muted);font-size:12px}.setupBodyText{height:88px;color:var(--text);white-space:pre-wrap}.setupDetails{color:var(--muted);line-height:1.6}.setupLabel{color:var(--text);margin-bottom:5px}.setupInput{height:24px;background:var(--input);border:1px solid var(--border);color:var(--text);padding:4px 7px;overflow:hidden;white-space:nowrap;text-overflow:ellipsis}.setupRow{display:grid;grid-template-columns:1fr 94px;gap:8px;align-items:center}.setupChecks{display:grid;gap:8px;margin-top:6px}.setupCheck{color:var(--text);display:flex;align-items:center;gap:7px}.setupBox{width:13px;height:13px;border:1px solid var(--border);background:var(--input);display:inline-block;position:relative}.setupBox.checked:after{content:"";position:absolute;left:3px;top:1px;width:5px;height:8px;border:solid var(--accent);border-width:0 2px 2px 0;transform:rotate(45deg)}.setupTextarea{background:var(--input);border:1px solid var(--border);height:calc(100% - 76px);padding:8px;color:var(--text);overflow:hidden;white-space:pre-wrap;font-family:Segoe UI,Arial,sans-serif;line-height:1.35}.setupLog{height:calc(100% - 132px);background:var(--log);border:1px solid var(--border);color:var(--logText);padding:8px;overflow:hidden}.setupButtonBar{grid-column:1/3;background:var(--footer);padding:8px;display:flex;justify-content:space-between;align-items:center}.setupButtonsRight{display:flex;gap:8px}.setupBtn{min-width:92px;height:30px;border:1px solid var(--buttonBorder);background:linear-gradient(var(--buttonGloss),var(--button));color:var(--buttonText);display:grid;place-items:center;box-shadow:inset 0 1px #ffffff55}.setupBtn.primary{border-color:var(--accentDown);background:linear-gradient(var(--accentGloss),var(--accent));color:var(--accentText)}.setupProgress{height:24px;background:linear-gradient(var(--progressBack),var(--input));border:1px solid var(--border);margin-bottom:8px;overflow:hidden}.setupProgress.small{height:20px}.setupProgress div{height:100%;background:linear-gradient(var(--accentGloss),var(--accentDown));box-shadow:inset 0 10px #ffffff45}.setupHiddenSidebar{display:none}
...
<div id="previewModal" class="previewModal"><div class="previewTop"><h3>Installer Preview</h3><label>Page</label><select id="previewPage" onchange="renderInstallerPreview()"><option value="welcome">Welcome</option><option value="license">License</option><option value="options">Options</option><option value="installing">Installing</option><option value="complete">Complete</option></select></div><div id="previewContent" class="previewStage"></div><div><button onclick="closePreview()">Close</button></div></div>
<script>
const $=id=>document.getElementById(id);let current={},previewState=null;
...
const themes={oldschool:{headerColor:'white',accentColor:'cyan',progressColor:'green',banner:'classic',style:'oldschool'},dark:{headerColor:'gray',accentColor:'cyan',progressColor:'green',banner:'minimal',style:'dark'},amber:{headerColor:'yellow',accentColor:'yellow',progressColor:'yellow',banner:'classic',style:'amber'},blue:{headerColor:'white',accentColor:'blue',progressColor:'cyan',banner:'classic',style:'blue'}};
function applyThemePreset(){const p=$('themePreset').value;if(!themes[p])return;const t=themes[p];$('headerColor').value=t.headerColor;$('accentColor').value=t.accentColor;$('progressColor').value=t.progressColor;$('banner').value=t.banner;$('windowStyle').value=t.style;setStatus('Theme preset applied.');}
function detectTheme(t,w){for(const [k,v] of Object.entries(themes)){if((w?.style||'')==v.style)return k;if((t.headerColor||'')==v.headerColor&&(t.accentColor||'')==v.accentColor&&(t.progressColor||'')==v.progressColor&&(t.banner||'')==v.banner)return k;}return 'custom';}
...
function previewInstaller(){previewState=manifest();renderInstallerPreview();$('previewModal').classList.add('open');}
function renderInstallerPreview(){const m=previewState||manifest();const p=resolveRuntimePalette(m);const page=$('previewPage')?.value||'welcome';const w=m.window||{};const width=clamp(parseInt(w.width||720,10),640,1100);const height=clamp(parseInt(w.height||460,10),420,780);const showSidebar=w.showSidebar!==false;const target=expandPath(m.defaultInstallDirectory||'{InstallDir}',m);const title=previewTitle(page,m,target);const sub=previewSubtitle(page,m,target);const css=paletteStyle(p,showSidebar);$('previewContent').innerHTML=`<div class="setupPreview" style="width:${width}px;height:${height}px;${css}"><div class="setupChrome"><span>${escapeHtml(title)}</span><div class="setupChromeBtns"><span></span><span></span><span></span></div></div><div class="setupClient"><div class="setupSidebar ${showSidebar?'':'setupHiddenSidebar'}">${renderPreviewSidebar(m,page)}</div><div class="setupContent"><div class="setupTitle">${escapeHtml(title)}</div><div class="setupSub">${escapeHtml(sub)}</div>${renderPreviewPage(page,m,target,p)}</div><div class="setupButtonBar"><div class="setupBtn">${page==='installing'?'Cancel':'Cancel'}</div><div class="setupButtonsRight"><div class="setupBtn">${page==='welcome'?'':'< Back'}</div><div class="setupBtn primary">${page==='options'?'Install':page==='complete'?'Finish':'Next >'}</div></div></div></div></div>`;}
function renderPreviewSidebar(m,page){const steps=[['welcome','Welcome'],['license','License'],['options','Options'],['installing','Install'],['complete','Finish']];return `<b>${escapeHtml(m.productName||'Setup')}</b><small>Setup Wizard</small><div class="setupSteps">${steps.map((s,i)=>`<div class="setupStep ${s[0]===page?'active':''}"><span class="setupStepDot">${s[0]===page?i+1:''}</span><span>${s[1]}</span></div>`).join('')}</div>`;}
function renderPreviewPage(page,m,target,p){if(page==='license'){const license=String(m.licenseText||'No license text configured.').slice(0,2600);return `<div class="setupTextarea">${escapeHtml(license)}</div>`;}if(page==='options'){const components=(m.components||[{id:'main',name:'Main Files',required:true,defaultSelected:true}]).map(c=>`<div class="setupCheck"><span class="setupBox checked"></span>${escapeHtml(c.name||c.id)}${c.required?' (required)':''}</div>`).join('');const shortcuts=(m.shortcuts||[]).map(s=>`<div class="setupCheck"><span class="setupBox checked"></span>${escapeHtml(shortcutLabel(s.location))}</div>`).join('');return `<div class="setupLabel">Destination folder</div><div class="setupRow"><div class="setupInput">${escapeHtml(target)}</div><div class="setupBtn">Browse...</div></div><div class="setupLabel" style="margin-top:18px">Components</div><div class="setupChecks">${components}</div>${shortcuts?`<div class="setupLabel" style="margin-top:18px">Shortcuts</div><div class="setupChecks">${shortcuts}</div>`:''}`;}if(page==='installing'){return `<div class="setupProgress"><div style="width:62%"></div></div><div class="setupDetails">Overall: 62% - 31/49 files</div><div class="setupProgress small"><div style="width:78%"></div></div><div class="setupDetails">Current file: 78% - ${escapeHtml((m.productName||'Application')+'.dll')}</div><div class="setupLog">Installing files...<br>Creating shortcuts...<br>Writing uninstall receipt...</div>`;}if(page==='complete'){const foot=expandTemplate((m.window||{}).footerText||'',m,target);return `<div class="setupBodyText">${escapeHtml((m.productName||'Application')+' has been installed successfully.')}</div><div class="setupDetails">${escapeHtml(foot)}</div>`;}const intro=expandTemplate((m.window||{}).introText||'This setup will install {ProductName} on your computer.',m,target);return `<div class="setupBodyText">${escapeHtml(intro+'\\n\\nSetup will install '+(m.productName||'Application')+' '+(m.version||'')+'.')}</div><div class="setupDetails">Publisher: ${escapeHtml(m.publisher||'')}<br>Files: package file count shown at runtime<br>Package: compressed size shown at runtime</div>`;}
function previewTitle(page,m,target){const w=m.window||{};if(page==='license')return'License Agreement';if(page==='options')return'Choose Install Location';if(page==='installing')return'Installing';if(page==='complete')return'Completing Setup';return expandTemplate(w.title||'{ProductName} Setup',m,target);}
function previewSubtitle(page,m,target){const w=m.window||{};if(page==='license')return'Please review the license terms before continuing.';if(page==='options')return'Select the destination folder and optional components.';if(page==='installing')return'Setup is installing files and configuring the application.';if(page==='complete')return expandTemplate(w.footerText||'',m,target);return expandTemplate(w.subtitle||'Install {ProductName} {Version}',m,target);}
function resolveRuntimePalette(m){const t=m.theme||{},w=m.window||{};let s=String(w.style||'').trim().toLowerCase();if(!s||s==='classic'){if(eq(t.banner,'minimal')&&eq(t.headerColor,'gray'))s='dark';else if(eq(t.accentColor,'yellow'))s='amber';else if(eq(t.accentColor,'cyan')&&eq(t.progressColor,'green'))s='oldschool';else s='blue';}if(s==='dark'||s==='modern-dark')return pal('#191d22','#12161b','#0c1116','#eef2f5','#dde2e6','#9eaab4','#ffffff','#8798a6',themeHex(t.accentColor,'#41c7a5'),'#4ad6b2','#2a967d','#000000','#262d34','#e8ecf0','#525e6a','#313a43','#20252b','#0e1114','#d7e1e6');if(s==='amber'||s==='terminal')return pal('#14110a','#1f190c','#30240b','#ffd67a','#f8e0ac','#c3a05c','#ffe8a6','#d2a553','#de9d2a','#f1b23c','#a97016','#000000','#2f2512','#ffe09e','#77561f','#3c2f17','#241d0f','#0d0b06','#ffda8b');if(s==='oldschool'||s==='retro')return pal('#e0e0e0','#c6c6c6','#000080','#000000','#000000','#404040','#ffffff','#b9d2ff','#008080','#009696','#006060','#ffffff','#d8d8d8','#000000','#606060','#e8e8e8','#ffffff','#ffffff','#000000');return pal('#ffffff','#f5f5f5','#1c3658','#000000','#202020','#464646','#ffffff','#b9d2e6',themeHex(t.accentColor,'#0078d7'),'#188ceb','#005ab4','#ffffff','#fafafa','#000000','#aaaaaa','#ebf3fc','#ffffff','#ffffff','#000000');}
function pal(content,footer,side,title,text,muted,sideText,sideMuted,accent,accentHover,accentDown,accentText,button,buttonText,border,buttonHover,input,log,logText){const sideGloss=blend(side,'#ffffff',.16),buttonGloss=blend(button,'#ffffff',.24),accentGloss=blend(accent,'#ffffff',.30),progressBack=blend(input,'#000000',.10);return{content,footer,side,title,text,muted,sideText,sideMuted,accent,accentHover,accentDown,accentText,button,buttonText,border,buttonHover,input,log,logText,sideGloss,buttonGloss,accentGloss,progressBack};}
function paletteStyle(p,showSidebar){return `--content:${p.content};--footer:${p.footer};--side:${p.side};--sideGloss:${p.sideGloss};--title:${p.title};--text:${p.text};--muted:${p.muted};--sideText:${p.sideText};--sideMuted:${p.sideMuted};--accent:${p.accent};--accentHover:${p.accentHover};--accentDown:${p.accentDown};--accentText:${p.accentText};--button:${p.button};--buttonGloss:${p.buttonGloss};--buttonText:${p.buttonText};--buttonBorder:${p.border};--input:${p.input};--log:${p.log};--logText:${p.logText};--border:${p.border};--caption:${p.side};--captionText:${p.sideText};--accentGloss:${p.accentGloss};--progressBack:${p.progressBack};--sidebarWidth:${showSidebar?'178px':'0px'};`;}
function themeHex(name,fallback){const map={cyan:'#0099bc',green:'#00965f',yellow:'#cd8f00',blue:'#0078d7',white:'#ffffff',gray:'#5a6066',grey:'#5a6066',red:'#ff6b6b'};return map[String(name||'').toLowerCase()]||fallback;}
function blend(a,b,amount){const x=hexRgb(a),y=hexRgb(b);return rgbHex(Math.round(x[0]+(y[0]-x[0])*amount),Math.round(x[1]+(y[1]-x[1])*amount),Math.round(x[2]+(y[2]-x[2])*amount));}
...
function eq(a,b){return String(a||'').toLowerCase()===b;}
function shortcutLabel(location){return String(location||'').toLowerCase()==='desktop'?'Create a Desktop shortcut':String(location||'').toLowerCase()==='startmenu'?'Create a Start Menu shortcut':String(location||'').toLowerCase()==='applications'?'Create an Applications menu shortcut':String(location||'').toLowerCase()==='install'?'Create a launcher in the install folder':'Create '+location+' shortcut';}
function expandTemplate(s,m,target=''){return String(s||'').replaceAll('{ProductName}',m.productName||'').replaceAll('{Version}',m.version||'').replaceAll('{Identifier}',m.identifier||'').replaceAll('{Publisher}',m.publisher||'').replaceAll('{InstallDir}',target);}
function expandPath(s,m){return expandTemplate(s,m,'{InstallDir}').replaceAll('{ProgramFiles}','C:\\Program Files').replaceAll('{LocalAppData}','%LOCALAPPDATA%').replaceAll('{AppData}','%APPDATA%').replaceAll('{Home}','%USERPROFILE%');}
